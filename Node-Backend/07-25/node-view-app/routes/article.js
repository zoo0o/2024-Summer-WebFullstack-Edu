...
//본 라우터 파일의 기본 호출 주소 : http://localhost:3000/article로 시작하게
//app.js,에서 라우터 파일 참조시 기본주소를 설정한다.
...
router.get('/list', async(req,res)=>{
   ...
   //**{속성:데이터} : {articles:articles} 속성명과 변수명이 같으면 생략할 수 있음
});
...
router.post('/create',async(req,res)=>{
   ...
   //Step1: 사용자 입력한 폼태그 내 입력/선택 데이터 추출하기
   //req.body.전달된 폼태그 내 HTML 입력/선택 요소의 name 속성명으로 지정
   const title = req.body.title;
   const contents = req.body.contents;
   const display = req.body.display;

   //Step2: DB 게시글 테이블에 저장할 JSON 데이터 생성하기
   //객체 속성명과 속성의 데이턱밧 변수/상수명이 같으면 상수/변수명은 생략 가능하다.
   const article = {
      title,
      contents:contents,
      display,
   ...
   //DB서버에서 Insert SQL구문을 통해서 DB 등록 처리가 되면 등록된 실제 데이터셋을 다시 반환함
   ...
   //Step4: 등록 완료 후 게시글 목록 페이지로 이동시킴
...
-호출 방식 : Post
-동작 결과 : 기존 게시글 정보를 수정 처리하고 목록 페이지로 이동시킨다.
*/
router.post('/modify', async(req,res)=>{
   //Step1: 사용자 수정 데이터를 추출하고 수정할 데이터 소스를 생성한다.
   //수정할 대상이 되는 게시글 고유번호
   const articleIdx = req.body.article_id;   //hidden태그의 name 속성 값
   ...
   //Update article Set title='수정한 제목', contents='수정한 내용', display='게시여부값', modify_id=1, modify_date='2024-07-25 18:08:12' WHERE as article_id=1;

   //수정이 완료되면 DB서버에서 수정처리 건수가 반환된다.
...
/*
-기존 게시글 데이터 삭제 처리 요청과 응답처리 라우팅메소드
-호출주소 : http://localhost:3000/article/delete?aid=1
-호출 방식 : Get
-동작 결과 : 해당 게시글 삭제 처리하고 목록페이지로 이동시킨다.
*/
...
-호출방식 : http://localhost:3000/article/modify/1
-호출 방식 : Get
-동작 결과 : DB에서 해당 단일게시글 정보를 조회해와서 지정 뷰파일에 데이터를 전달하고 뷰파일 내에서 해당 데이터를 html태그에
-출력해서 최조 웹브라우저에 동적으로 변경된 웹페이지를 반환한다.
*/
router.get('/modify/:idx', async(req,res)=>{
   //Step1: url주소에서 게시글 고유번호를 추출한다.
   const articleIdx = req.params.idx;

   //Step2: DB 게시글 테이블에서 해당 게시글 고유번호에 해당하는 단일게시글 정보를 조회해온다.
   //조회했다고 가정한다.
   const article =    {
      article_id:"1",
      title:"게시글 제목1입니다.",
   ...
   //Step3: DB에서 가져온 단일게시글 정보를 modify.ejs 뷰파일 전달합니다.
